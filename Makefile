.PHONY: help test coverage build install fmt fmt-fix vet mod-tidy lint lint-fix all clean

.DEFAULT_GOAL := help

BINARY_NAME := zen-notes
MAIN_PACKAGE := .
...
lint: fmt mod-tidy vet ## Run all linting checks (gofmt, mod-tidy, go vet, golangci-lint)
