...
`docs/keys.md` is the other one worth a look on any range touching
`internal/editor`. It is the longest document here and the one a reader checks
against the app in front of them, so a binding that moved and a table that did
not is the kind of wrong somebody notices immediately.
...
There is no Makefile, so the gate is the commands CI runs, all four green:
...
git tag -a vX.Y.Z -m "zen-notes vX.Y.Z" && git push origin vX.Y.Z
```
...
- The installed binary reports the version rather than `dev`, via `-version`. A `dev` here means
  the ldflags stamp broke, and the binary is otherwise fine, which is why nothing
  else catches it.
- Open it and write a line, not just `-version`. A release that starts and
  cannot save is one nobody checked.
